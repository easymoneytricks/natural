...
import { CustomerAddresses, CustomerProfile } from './pages/CustomerAccountPages'
import { CustomerOrders, CustomerOrderDetail } from './pages/CustomerOrders'

const informationalRoutes = ['/about', '/journal', '/new-arrivals', '/best-sellers', '/skin-types', '/concerns', '/gift-cards', '/contact', '/faq', '/shipping', '/returns', '/track-order', '/privacy', '/terms', '/refund-policy', '/cancellation-policy']
...
export default function App() {
  ...
  useEffect(() => {
    document.body.classList.toggle('overlay-open', searchOpen || cartOpen || menuOpen)
  ...
  return <>
    ...
    <main>
      <Routes>
        ...
        {informationalRoutes.map((path) => <Route key={path} path={path} element={<RouteShell title={path.slice(1)} />} />)}
      </Routes>
    </main>
    <CompareTray />
