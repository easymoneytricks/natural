...
export function WishlistProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const ids = guestItems.map((item) => item.id).filter(Boolean);
        const result = ids.length
          ? await commerceApi.mergeWishlist(authFetch, ids)
          : await commerceApi.getWishlist(authFetch);
        if (!active) return;
        setItems(decorateWishlist(result.data.items || result.data || []));
        setServerMode(true);
        if (ids.length) setGuestItems([]);
      } catch {}
    };
    load();
    return () => {
  ...
  const toggle = async (product, metadata = {}) => {
    const existing = items.find((item) => item.slug === product.slug);
    if (serverMode && product.id) {
      const result = existing
        ? await commerceApi.removeWishlist(authFetch, product.id)
        : await commerceApi.addWishlist(authFetch, product.id);
    ...
    setGuestItems((current) =>
      existing
        ...
        : [...current, { id: product.id, slug: product.slug, ...metadata }],
    );
  };
  const value = useMemo(
    () => ({
      ...
      toggle,
      remove: async (slug) => {
        const line = items.find((item) => item.slug === slug);
        if (serverMode && line?.id) {
          const result = await commerceApi.removeWishlist(authFetch, line.id);
          setItems(result.data || []);
          return;
        }
        setGuestItems((current) =>
      ...
      clear: async () => {
        if (serverMode) {
          for (const line of items)
            if (line.id) await commerceApi.removeWishlist(authFetch, line.id);
          setItems([]);
          return;
        }
        setGuestItems([]);
...
export function CompareProvider({ children }) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("natural-beauty-compare", JSON.stringify(items));
    } catch {
  ...
  const toggle = (product) =>
    setItems((current) => {
      ...
      if (current.length >= 4) {
        setMessage(
          "You can compare up to 4 products. Remove one product to add another.",
        );
