...
function makeVariants(product, detail) {
  ...
  if (detail.variants)
    return detail.variants.map(
      ([sku, size, skinType, concern, mrp, price, stock]) => ({
        sku,
        attributes: { size, skinType, concern },
        mrp,
        price,
        stock,
        image: product.image,
  ...
  return product.sizes.map((size) => ({
    ...
    mrp: product.mrp,
    price: product.price,
    stock: product.availability === "out-of-stock" ? 0 : 10,
    image: product.image,
  }));
}
...
export function ProductDetail() {
    ...
    getProductBySlug(slug, { signal: controller.signal })
      ...
      .catch((error) => {
        if (error?.name !== "AbortError" && !controller.signal.aborted)
          setLoadError(
            error?.code === "PRODUCT_NOT_FOUND" ? "not-found" : "error",
          );
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    setSelection(
      dimensions.length
        ? { [dimensions[0]]: variants[0]?.attributes[dimensions[0]] }
        : {},
    );
    // oxlint-disable-next-line react/set-state-in-effect
    setQuantity(1);
        ...
        .filter((variant) =>
          dimensions
            ...
            .every(
              (item) =>
                !selection[item] ||
                variant.attributes[item] === selection[item],
            ),
        )
        .map((variant) => variant.attributes[dimension]),
    ),
  ];
    ...
    dimensions.slice(dimensions.indexOf(dimension) + 1).forEach((later) => {
      if (
        !variants.some((variant) =>
          dimensions.every(
            (item) => !next[item] || variant.attributes[item] === next[item],
          ),
        )
      )
        delete next[later];
  ...
  const mrp =
    selectedVariant?.mrp || Math.min(...variants.map((variant) => variant.mrp));
  ...
  return (
    <>
        ...
        <div className="product-main">
          <section className="product-gallery">
            <div className="gallery-thumbs">
              {gallery.map((image, index) => (
                <button
                  key={image}
                  className={galleryIndex === index ? "is-selected" : ""}
          ...
          <section className="purchase-panel">
            ...
            <div className="detail-price">
              <strong>
                {selectedVariant ? money(price) : `From ${money(price)}`}
              </strong>
              {mrp > price && (
                <>
                  <s>{money(mrp)}</s>
                ...
                <fieldset key={dimension}>
                  ...
                  <div className="variant-options">
                    {[
                      ...new Set(
                        variants.map(
                          (variant) => variant.attributes[dimension],
                        ),
                      ),
                    ].map((value) => {
                      const enabled =
              ...
              {!selectedVariant
                ...
                : stock === 0
                  ...
                  : stock <= 3
                    ? `Only ${stock} left`
                    : "IN STOCK"}
              {selectedVariant && <small>SKU: {selectedVariant.sku}</small>}
            </div>
            ...
            <div className="secondary-actions">
              <button
                onClick={() =>
                  wishlist.toggle(product, {
                    preferredSku: selectedVariant?.sku,
                    preferredAttributes: selectedVariant?.attributes,
                  })
                }
              ...
              >
                ...
                />{" "}
                {wishlist.has(product.slug) ? "Wishlisted" : "Wishlist"}
              </button>
              <button
                onClick={() => compareList.toggle(product)}
                aria-pressed={compareList.has(product.slug)}
              >
                <Scale size={17} />{" "}
                {compareList.has(product.slug)
                  ? "Added to compare"
                  : "Add to compare"}
              </button>
            </div>
            <div className="delivery-check">
              <p>
                <MapPin size={15} /> Delivery options
              </p>
              <div>
                <input
                  value={pin}
                  ...
                  inputMode="numeric"
                />
                <button onClick={() => setDelivery(/^\d{6}$/.test(pin))}>
                  Check
                </button>
              </div>
              {delivery === true && (
                <small>
                  Delivery estimate: 3–5 business days · Cash on Delivery
                  available · Complimentary shipping on orders above ₹999.
                </small>
              )}
              {delivery === false && (
          ...
          {[
            ...
            <div key={title}>
              <button
                ...
                onClick={() => setOpenInfo(openInfo === title ? "" : title)}
              >
                <span>{title}</span>
                <Plus size={16} />
              </button>
      ...
      <div className="mobile-sticky-buy">
        <span>{selectedVariant ? money(price) : "Choose options"}</span>
        <button disabled={!selectedVariant || stock < 1} onClick={addToBag}>
          {!selectedVariant
            ? "Choose options"
            : stock === 0
              ? "Out of stock"
    ...
    <section className="reviews-section">
      ...
      <div className="review-bars">
        {[5, 4, 3, 2, 1].map((star, index) => (
          <p key={star}>
            ...
            <i>
              <b style={{ width: `${[92, 6, 1, 0, 1][index]}%` }} />
            </i>
          </p>
        ))}
      </div>
      <div className="review-quotes">
        <blockquote>
          ...
          <cite>Aanya M. · 50 ml · Dry · Dryness</cite>
        </blockquote>
        <blockquote>
          “My skin feels calm and soft by morning.”
          <cite>Meera K. · 100 ml · Sensitive · Barrier Support</cite>
        </blockquote>
        <blockquote>
          “A lovely everyday layer under sunscreen.”
          <cite>Rhea S. · 50 ml · Combination · Barrier Support</cite>
        </blockquote>
      </div>
    </section>
  );
