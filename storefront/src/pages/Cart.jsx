...
function PromoBox({ title, input, setInput, action, onAction, message, applied, onRemove, gift }) { return <section className="promo-box"><p>{title}</p>{applied ? <div className="applied-promo"><div><strong>{gift ? 'Gift card' : applied.code}</strong><span>{gift ? `Available balance: ${money(applied.balance)}` : `${applied.label} · You saved ${money(0)}`}</span></div><button onClick={onRemove}>Remove</button></div> : <div className="promo-input"><label className="sr-only" htmlFor={`${action}-${title}`}>{title}</label><input id={`${action}-${title}`} value={input} onChange={(event) => setInput(event.target.value)} placeholder={gift ? 'Enter gift card code' : 'Enter coupon code'} /><button onClick={onAction}>{action}</button></div>}{message && <small className="promo-error" role="status">{message}</small>}</section> }
