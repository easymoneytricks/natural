...
const fallbackGroups = [
  ...
  },
  {
    ...
    labels: ["Normal", "Dry", "Oily", "Combination", "Sensitive", "Acne-Prone"],
  },
  {
    key: "concern",
    title: "Concern",
  ...
  },
  {
    key: "size",
    title: "Pack size",
    options: ["30-ml", "50-ml", "100-ml", "200-ml"],
    labels: ["30 ml", "50 ml", "100 ml", "200 ml"],
  },
  {
    key: "price",
    title: "Price",
    options: ["under-500", "500-799", "800-999", "1000-1499", "1500-plus"],
    labels: [
      "Under ₹500",
      "₹500 – ₹799",
      "₹800 – ₹999",
...
export function Shop() {
  ...
  useEffect(() => {
    ...
    getCatalogFilters({ signal: controller.signal })
      .then((payload) => {
        ...
        {activeFilters.length > 0 && (
          <div className="active-filters">
            ...
            {activeFilters.map(({ key, value }) => (
              <button
                key={`${key}-${value}`}
                onClick={() => updateParams(key, value, false)}
              >
                {filterGroups.find((group) => group.key === key)?.labels?.[
                  filterGroups
              ...
              <div className="shop-grid">
                {products.map((product) => (
                  <ProductCard
                    key={product.slug}
                    product={product}
                    onChooseOptions={() => navigate(`/product/${product.slug}`)}
                  />
                ))}
              </div>
            ) : (
              <EmptyState clearFilters={clearFilters} search={search} />
            )}
          </section>
        </div>
        {meta.totalPages > 1 && (
          <nav className="pagination" aria-label="Product pages">
            {Array.from(
              ...
              <button
                className={
                  Number(searchParams.get("page") || 1) === page ? "active" : ""
                }
                key={page}
                onClick={() => {
                  const next = new URLSearchParams(searchParams);
...
function FilterPanel({
  ...
  return (
    <div className="filter-panel">
      {filterGroups.map(({ key, title, options, labels }) => (
        <section key={key}>
          <button
            className="filter-group-heading"
            aria-expanded={openGroups[key] || false}
            onClick={() =>
              ...
              {options.map((option, index) => (
                <label key={option}>
                  <input
                    ...
                    onChange={(event) =>
                      updateParams(key, option, event.target.checked)
                    }
                  />
