...
Create a separate production secret set in the hosting provider. Start from `backend/.env.production.example`; do not copy the development `.env` or commit a populated env file. Generate independent 64+ character random values for `JWT_ACCESS_SECRET` and `ADMIN_JWT_ACCESS_SECRET`. Keep Cashfree, SMTP, database and media credentials server-side only.
